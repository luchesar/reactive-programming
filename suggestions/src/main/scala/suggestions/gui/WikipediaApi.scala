...
import rx.lang.scala.{Subscription, Observer, Subject, Observable}
  ...
  implicit class ObservableOps[T](obs: Observable[T]) {
    ...
    def timedOut(totalSec: Long): Observable[T] =
      Observable[T]((observer: Observer[T]) => {
        ...
        obs.subscribe(
          x => {
            if ((time - start)/1000 < totalSec) observer.onNext(x)
    ...
    def concatRecovered[S](requestMethod: T => Observable[S]): Observable[Try[S]] = {
      Observable[Try[S]]((observer: Observer[Try[S]]) => {
        obs.subscribe(
          x => {
            requestMethod(x).subscribe(
              sx => observer.onNext(Success(sx)),
              sex => observer.onNext(Failure(sex))
            )
          },
          ex => observer.onError(ex),
          () => observer.onCompleted()
