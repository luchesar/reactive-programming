...
trait NodeScala {
  ...
  def start(relativePath: String)(handler: Request => Response): Subscription = {
    ...
    val subscr = listener.start
    def newRequestHandler(cancel: CancellationToken): Future[Unit] = {
      Future {
        val next = listener.nextRequest
        next.flatMap(re => Future[(Response, Exchange, CancellationToken)] {
          if (cancel.nonCancelled) {
            newRequestHandler(cancel)
            (handler(re._1), re._2, cancel)
          } else {
            subscr.unsubscribe
            throw new Exception()
          }
        }).flatMap(rec => Future[Unit] {
          respond(rec._2, rec._3, rec._1)
        })
        
      }
    }
    
    Future.run() { cancel => newRequestHandler(cancel)}
  }
}
...
object NodeScala {
  ...
  trait Listener {
    ...
    def nextRequest(): Future[(Request, Exchange)] = {
      ...
      val contenct = createContext(e => {
        promise.success((e.request, e))
        Listener.this.removeContext
      })
      promise.future
    }
  }
