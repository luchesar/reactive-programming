...
class NodeScalaSuite extends FunSuite {
  ...
  test("all future combinator") {
    val all = Future.all(List(Future{0}, Future{1}, Future(2), Future{3}))
    assert(Await.result(all, 1 hour) == List(0, 1, 2, 3))
  }
  ...
  test("Server should serve requests") {
    ...
    def test(req: Request) {
      ...
      val content = Await.result(webpage.loaded.future, Duration.Inf)
